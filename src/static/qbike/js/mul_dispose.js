        ...
        _api: function(_base_url) {
            ...
            return {
                ...
                product: {
                    ...
                    openLock: _base_url + '/batchInstruction/openLock',
                    openBeap: _base_url + '/batchInstruction/openBeep',
                    setHostPort: _base_url + '/batchInstruction/setHostPort',
...
var dispatch_obj = {
    ...
    componentInit: function() {
        ...
        var click_flag = true;
        _obj = this;
        $("#flow_sel").off().on("click",function(){
            if($(this).is(':checked'))_obj.drawFlow();
            else _obj.drawArea();
        });
        $("#draw_line").off().on("click",function(){
            _obj.drawLine();
        });
        ...
        $("#show_list").off().on("click",function(){
                $.ajax({
                    url: _defautBiz.api("listArea"),
                    dataType: "json",
                    method: 'get',
        ...
        $("#save_confirm").off().on("click",function(){
    ...
    dataInit: function() {
        ...
        function typeChange(){
            if($('.lock-type').val()=="nocklock"){ $("#select_normal").show();$("#select_om").hide();$("#select_normal").attr("choose","right");$("#select_om").attr("choose","wrong")}
            else { $("#select_normal").hide();$("#select_om").show();$("#select_normal").attr("choose","wrong");$("#select_om").attr("choose","right")}
        }
        typeChange();
        $(".lock-type").on("change",function(){
            typeChange();
        })
...
function mul_dispose(){
    ...
    var type =$("#select_normal").attr("choose")=="right"?$('#select_normal').val(): $('#select_om').val(),
        ip = $('.ip-address').val(),
        port = $('.port-num').val(),
        time = $('.time-num').val(),
        version = $('.version-number').val(),
        locktype=$('.lock-type').val(),
...
function sendMsg(url, opts){
    $.ajax({
        ...
        success: function(data) {
            if(data.success){
                $('#a_info').text(data.message);
                $('#action_success').modal('show');
            }
            else{
                $('#fs_info').text(data.message);
...
$('#save_judge').off().on('click',function(){
    //根据不同的type来区分具体的操作,开关锁,开关蜂鸣器,修改服务器地址,并且根据区分后的操作来添加对应的参数,调对应的接口
    var type = $(this).attr('data-type'),
        lockIds = $(this).attr('data-ids'),
        port = $(this).attr('data-port'),
        host = $(this).attr('data-ip'),
        time = $(this).attr('data-time'),
        version = $(this).attr('data-version'),
        lockType=$(this).attr('data-lockType'),
        upgradeType = $(this).attr('data-upgradeType');
    $('#judge_alert').modal('hide');
    //此处应注意,在操作完成后,要删除掉确认按钮上添加的几个attr属性. $().removeAttr();
    var url, opts;
    switch (type){
        case "1":
        ...
        case "5":
            ...
            break;
        case "6":
            url = _defautBiz.api("transportation");
            opts = {
                lockIds: lockIds
            }
            break;
        case "7":
            url = _defautBiz.api("setOpenTime");
            opts = {
                lockIds: lockIds,
                time: time
            }
            break;
        case "8":
            url = _defautBiz.api("setCloseTime");
            opts = {
                lockIds: lockIds,
                time: time
            }
            break;
        case "9":
            url = _defautBiz.api("getParaMessage");
            opts = {
                lockIds: lockIds,
                brand:lockType
            }
            break;
        case "10":
            url = _defautBiz.api("versionQuery");
            opts = {
                lockIds: lockIds,
                brand:lockType
            }
            break;
        case "11":
            url = _defautBiz.api("updateVersion");
            opts = {
                lockIds: lockIds,
                version: version,
                updateType: upgradeType
            }
            break;
        default:
            return;
