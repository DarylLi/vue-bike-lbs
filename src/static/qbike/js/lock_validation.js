...
var _defautBiz =
    _defautBiz || {
        ...
        _api: function(_base_url) {
            ...
            return {
                debug: {
                    getListData: _base_url + '/lock_validation_data.json' + "?" + _v,
                    checkStatus: _base_url + '/action_success.json' + "?" + _v,
                    getManufacturerList: _base_url + '/manufac.json' + "?" + _v,
                ...
                product: {
                    getListData: _base_url + '/lockCheck/listPage' + "?" + _v,
                    checkStatus: _base_url + '/lockCheck/checkStatus' + "?" + _v,
                    checkDel: _base_url + '/lockCheck/del',
                    getManufacturerList: _base_url + '/lockCheck/getManufacturerList' + "?" + _v,
...
var dispatch_obj = {
    ...
    initManuc: function(){
        var manuObj=document.getElementById('manuFactury-List');
        $.ajax({
            url: _defautBiz.api("getManufacturerList"),
            dataType: "json",
            method: 'get',
            success: function(data) {
                if(data.success && data.data){
                    var manuList = data.data;
                    manuObj.options.length = 0;
                    manuObj.options.add(new Option("全部"),"");
                    for(var i = 0; i < manuList.length;i++){
                        manuObj.options.add(new Option(manuList[i]),""); //这个兼容IE与firefox
                    }
                }
            }});
    },
    ...
    parseNextPageData: function(data){
        ...
        for(var i = 0;i < pageSize && i < data.datas.length; i++){
            if(parseInt(data.datas[i].checkStatus) == 0){
                data_append+="<tr><td>"+(data.datas[i].lockId)+"</td><td>" + data.datas[i].manufacturer + "</td><td>" + data.datas[i].provinceArea + "</td><td>" + data.datas[i].cityArea + "</td><td>" + data.datas[i].uploadTimeStr + "</td>"+ "<td>" + data.datas[i].checkStatusStr +"</td>"+ "<td>"+(data.datas[i].authCode?"是":"否")+ "</td><td><a href='javascript:void(0)' class='action-allow' del_num='"+data.datas[i].lockId+"' style='margin-right:5px '>删除</a></td></tr>";
            }else{
                data_append+="<tr><td>"+(data.datas[i].lockId)+"</td><td>" + data.datas[i].manufacturer + "</td><td>" + data.datas[i].provinceArea + "</td><td>" + data.datas[i].cityArea + "</td><td>" + data.datas[i].uploadTimeStr + "</td>" +"</td>" + data.datas[i].checkStatusStr +"</td>"+ "<td>"+(data.datas[i].authCode?"是":"否")+ "</td><td><a href='javascript:void(0)' class='action-allow' del_num='"+data.datas[i].lockId+"' style='margin-right:5px '>删除</a></td></tr>";
            }
        }
        $("#Pagination tbody").append(data_append);
        $(document.body).on('click','.action-allow,.action-deny',function(){
            var status = $(this).attr('data-status');
            //var lockId = $(this).parent().parent().find('td:eq(0)').html();
            var lockId = $(this).attr("del_num");
            _del_obj=this;
            $.ajax({
                url: _defautBiz.api("checkDel"),
                dataType: "json",
                method: 'get',
                data: {
                    lockId: lockId
    ...
    parseData: function(data){
        ...
        for(var i = 0;i < pageSize && i < data.datas.length; i++){
            if(parseInt(data.datas[i].checkStatus) == 0){
                data_append+="<tr><td>"+(data.datas[i].lockId)+"</td><td>" + data.datas[i].manufacturer + "</td><td>" + data.datas[i].provinceArea + "</td><td>" + data.datas[i].cityArea + "</td><td>" + data.datas[i].uploadTimeStr + "</td>" + "<td>"+(data.datas[i].authCode?"是":"否")+"</td><td><a href='javascript:void(0)' class='action-allow' del_num='"+data.datas[i].lockId+"' data-status='1' style='margin-right:5px '>删除</a></td></tr>";
            }else{
                data_append+="<tr><td>"+(data.datas[i].lockId)+"</td><td>" + data.datas[i].manufacturer + "</td><td>" + data.datas[i].provinceArea + "</td><td>" + data.datas[i].cityArea + "</td><td>" + data.datas[i].uploadTimeStr + "</td>" + "<td>"+(data.datas[i].authCode?"是":"否")+ "</td><td><a href='javascript:void(0)' class='action-allow' del_num='"+data.datas[i].lockId+"' data-status='1' style='margin-right:5px '>删除</a></td></tr>";
            }
        }
        ...
        $('.M-box1').pagination({
            ...
            coping:true,
            callback:function(index){
                var lockName = $('.lock-name').val();
                var status = $('.validate-status').val();
                var manufacturer = $('.produce-name').val();
                var provinceDom = document.getElementById('pro');
                var provinceArea = provinceDom.options[provinceDom.selectedIndex].text;

                var cityDom = document.getElementById('city');
                var cityArea = cityDom.options[cityDom.selectedIndex].text;

                var uploadStartTime = $('#createTime_from').val();
                var uploadEndTime = $('#createTime_to').val();
                var checkStartTime = $('#verifyTime_from').val();
                var checkEndTime = $('#verifyTime_to').val();
                var num=index.getCurrent();
    ...
    updateList:function(indx){
        ...
        for(var i=0;i<indx;i++)
            data_append+="<tr><td>"+("Misc"+i)+"</td><td>IE Mobile</td><td>Windows Mobile 6</td><td>-</td><td>C</td><td>C</td><td>C</td><td>C</td><td><a href='javascript:void(0)' style='margin-right:5px '>删除</a></td></tr>";
        $("#Pagination tbody").append(data_append);
        $('.M-box1').pagination({
            totalData:100,
            showData:indx,
            ...
            callback:function(index){
                ...
                data_append="";
                for(var i=0;i<indx;i++)
                    data_append+="<tr><td>"+("Misc"+i+num)+"</td><td>IE Mobile</td><td>Windows Mobile 6</td><td>-</td><td>C</td><td>C</td><td>C</td><td>C</td><td><a href='javascript:void(0)' style='margin-right:5px '>删除</a></td></tr>";
                $("#Pagination tbody").append(data_append);
            }
        });
    ...
    componentInit: function() {
        ...
        //
        //    $.ajax({
        //        url: _defautBiz.api("checkStatus"),
        //        dataType: "json",
        //        method: 'get',
        //        data: {
        //            lockId: lockId,
