...
var _defautBiz =
    _defautBiz || {
        ...
        _api: function(_base_url) {
            ...
            return {
                debug: {
                    getListData: _base_url + '/instruct_record_data.json' + "?" + _v,
                    ...
                    list:'/track/list',
                    saveArea:'/track/list',
                    openLock: _base_url + '/action_success.json' + "?_v=" + _v,
                    openBeap: _base_url + '/action_success.json' + "?_v=" + _v,
                    setHostPort: _base_url + '/action_success.json' + "?_v=" + _v,
                    setOpenLockBack: _base_url + '/action_success.json' + "?_v=" + _v,
                    setCloseLockBack: _base_url + '/action_success.json' + "?_v=" + _v,
                    getParamMessage: _base_url + '/action_success.json' + "?_v=" + _v,
                    export: _base_url + '/action_success.json' + "?_v=" + _v,
                    searchLockIds: _base_url + '/lockStatus/searchLockIds' + "?_v=" + _v,
                },
                product: {
                    getListData: _base_url + '/lockInstructions/listPage' + "?" + _v,
                    ...
                    openBeap: _base_url + '/instruction/openBeep',
                    ...
                    export: _base_url + '/lockInstructions/export?',
                    searchLockIds: _base_url + '/lockStatus/searchLockIds' + "?_v=" + _v,
                }
            }
        }
...
var dispatch_obj = {
    ...
    execStatus: "",
    init: function() {
        dispatch_obj.startTime = new Date().format("yyyy-MM-dd")
        dispatch_obj.endTime = new Date().format("yyyy-MM-dd")
        var params = {
            startTime: dispatch_obj.startTime,
            endTime: dispatch_obj.endTime,
            pageSize: 10,
            currPage: 1
        };
        ...
        // var data_append="";
        // for(var i=0;i<10;i++)
        //     data_append+="<tr><td>"+("Misc"+i)+"</td><td>IE Mobile</td><td>Windows Mobile 6</td><td>-</td><td>C</td><td>C</td><td>C</td><td>C</td><td><a href='javascript:void(0)' style='margin-right:5px '>通过</a><a href='javascript:void(0)'>不通过</a></td></tr>";
    ...
    initData: function(params){
        ...
        $.ajax({
            ...
            data: {
                ...
                stratTime: params.startTime && params.startTime,
                endTime: params.endTime && params.endTime,
                pageSize: params.pageSize,
                currPage: params.currPage,
                execStatus: params.hasOwnProperty("execStatus") ? params.execStatus : "",
            },
    ...
    parseNextPageData: function(data){
        ...
        for(var i = 0;i < pageSize && i < data.datas.length; i++)
            data_append+="<tr><td>"+(data.datas[i].id)+"</td><td>" + data.datas[i].lockId + "</td><td>" + data.datas[i].code + "</td><td>" + data.datas[i].intervalTime + "</td><td>" + data.datas[i].apn + "</td><td>" + data.datas[i].ip + "</td><td>" + data.datas[i].port + "</td><td style='max-width: 200px;word-break:break-all'>" + data.datas[i].answerContent + "</td><td>" + data.datas[i].execuStatusStr + "</td><td>" + data.datas[i].createdDateStr + "</td></tr>";
        $("#Pagination tbody").append(data_append);
    },
    parseData: function(data){
        ...
        $('.M-box1').pagination({
            ...
            callback:function(index){
                var num=index.getCurrent();
                var params = {
                    lockName: dispatch_obj.lockName?dispatch_obj.lockName:'',
                    startTime: dispatch_obj.startTime?dispatch_obj.startTime:'',
                    endTime: dispatch_obj.endTime?dispatch_obj.endTime:'',
                    pageSize: pageSize,
                    currPage: num,
                    execStatus: dispatch_obj.execStatus,
                };
    ...
    componentInit: function() {
        ...
        $('.query-list').on('click',function(){
            var lockName =dispatch_obj.lockName= $('.lock-name').val();
            var startTime =dispatch_obj.startTime= $('#createTime_from').val();
            var endTime  =dispatch_obj.endTime= $('#createTime_to').val();
            var statusSelect = dispatch_obj.execStatus = $('.status-select').val();
            ...
            else dispatch_obj.initData(params);
        });
...
function fileSelected() {
    ...
    if (file) {
        ...
        document.getElementById('file').innerHTML = 'Name: ' + file.name;
        document.getElementById('fileSize').innerHTML = 'Size: ' + fileSize;
        document.getElementById('fileType').innerHTML = 'Type: ' + file.type;
    }
}
...
//         //发送文件和表单自定义参数
//         xhr.open("POST", "/lockCheck/importExcel");
//         xhr.send(fd);
//     }
// }
function uploadFile() {
    // if(document.getElementById('file').files[0]){
    //     var form = document.getElementById("importfileForm");
    ...
    // $.ajax({
    //     url:"/lockCheck/importExcel",
    //     dataType:"json",    //返回的数据类型 json html 等
    //     type:"post", //请求方式 post get
    //     data:$("#importfileForm").serialize(), //传递form的表单参数
    //     success:function(data){
    //         //处理返回的数据
    //     }
    // });
...
$("#save_judge").on('click',function(){
    var lockId = $('.lock-name').val();
    var lockName =dispatch_obj.lockName= $('.lock-name').val();
    var startTime =dispatch_obj.startTime= $('#createTime_from').val();
    var endTime  =dispatch_obj.endTime= $('#createTime_to').val();
    var statusSelect = dispatch_obj.execStatus = $('.status-select').val();
    $("#judge_alert").modal('hide');
    //var url=dispatch_obj.lockName?("lockName="+lockId+"&startTime="+dispatch_obj.startTime+"&endTime="+dispatch_obj.endTime+"&execStatus="+dispatch_obj.execStatus):("lockName="+lockId);
    var url="lockName="+lockId+"&stratTime="+dispatch_obj.startTime+"&endTime="+dispatch_obj.endTime+"&execStatus="+dispatch_obj.execStatus;
