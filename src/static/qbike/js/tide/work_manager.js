...
var Manager = {
    ...
    bikeNum:"",
    user:"",
    userId:(getUrlParam("userId")?getUrlParam("userId"):""),
    init: function () {
        $.ajax({
        ...
        $.ajax({
            ...
            success: function(data) {
                if(data.success && data.data){
                    ...
                    userObj.options.add(new Option("请选择",""),"");
    ...
    parseNextPageData: function (data) {
        ...
        for (var i = 0; i < pageSize && i < data.datas.length; i++)
            data_append += "<tr><td><input type='checkbox' name='lockId' value='"+data.datas[i].id+"'/><td><span class='task_option' ot_user='"+data.datas[i].userId+"' ot_status='"+data.datas[i].taskStatus+"' ot_remark='"+data.datas[i].remark+"' ot_type='"+data.datas[i].bikeType+"' bike_num='"+data.datas[i].bikeId+"' task-id='"+data.datas[i].id+"'><img src='/static/qbike/img/normal.png' alt=''></span></td><td><span data-toggle='tooltip' data-original-title='"+data.datas[i].remark+"'>" + data.datas[i].bikeId + "</span></td><td>" + data.datas[i].userName + "</td><td>" + data.datas[i].taskTypeStr + "</td><td>" + data.datas[i].taskStatusStr + "</td><td>" + data.datas[i].lastUser + "</td><td>" + data.datas[i].lastUseStartTime + "</td><td>" + data.datas[i].lockUploadTime +"</td><td>" + data.datas[i].power + "</td>" +"<td>" + data.datas[i].fixedType + "</td>"+ "<td>" + data.datas[i].fixedTime + "</td>" +"<td><a href='/tide/mission_dispatch.html?lng="+data.datas[i].fixedLng+"&lat="+data.datas[i].fixedLat+"'>" + data.datas[i].fixedLocStreet + "</a></td><td><a href='/tide/mission_dispatch.html?lng="+data.datas[i].fixedLng+"&lat="+data.datas[i].fixedLat+"'>"+ (data.datas[i].locStreet?data.datas[i].locStreet:'添加') + "</a></td></tr>";
    ...
    parseData: function (data) {
        ...
        $("[data-toggle='tooltip']").tooltip();
        $(".task_option").on("click",function(){
            var type=$(this).attr("ot_type");
            var status=$(this).attr("ot_status");
            var user=$(this).attr("ot_user");
            var remark=$(this).attr("ot_remark");
           $("#option-type").val(type);
            $("#option-state").val(status);
            $("#option-user").val(user);
            $("#area_msg").val(remark);
            $("#option_panel").animate({"opacity":1});
            $("#option_panel").show();
            $("#option_num").val($(this).attr("bike_num"));
...
$("#save_judge").on('click',function(){
    ...
    //    bikeNum:_obj.bikeNum,
    //    user:_obj.user
    var url="?taskType="+Manager.mission_type+"&taskStatus="+Manager.mission_state+"&bikeId="+Manager.bikeNum+"&userName="+Manager.user+"&userId="+Manager.userId;
