    ...
    _defautBiz || {
        ...
        _api: function(_base_url) {
            ...
            return {
                ...
                product: {
                    history_list:"/lockHistoryStatus/listPage",
                    export:"/lockHistoryStatus/export",
                    searchLockIds: '/lockStatus/searchLockIds' + "?_v=" + _v,
                }
            }
        }
...
var history_obj = {
    ...
    updateList:function(indx){
        ...
        for(var i=0;i<indx;i++)
            data_append+="<tr><td>"+("Misc"+i)+"</td><td>I1</td><td>3 1 6</td><td>-</td><td>C</td><td>C</td><td>C</td><td>C</td><td>C</td><td>C</td><td>C</td><td>C</td><td>C</td><td>C</td><td>C</td><td>C</td><td>C</td><td>122</td></tr>";
    ...
    componentInit: function() {
        ...
        $("#query").on("click",function(){
            if(new Date($("#createTime_to").val().replace(/\-/g, "\/"))<new Date($("#createTime_from").val().replace(/\-/g, "\/"))){$("#s_info").text("起始时间必须小于结束时间");$("#search_rslt").modal('show')}
            else if($("#lockId").val()==""||$("#createTime_to").val()==""||$("#createTime_from").val()==""){$("#s_info").text("请补全筛选项!");$("#search_rslt").modal('show')}
            else _obj.refreshHistory();
        });
        ...
        $("#lockOption").on("click",function () {
            if(new Date($("#createTime_to").val().replace(/\-/g, "\/"))<new Date($("#createTime_from").val().replace(/\-/g, "\/"))){$("#s_info").text("起始时间必须小于结束时间");$("#search_rslt").modal('show')}
                ...
                //$.ajax({
                //    url: _defautBiz.api("history_list")+"?pageSize="+10000+"&currPage="+1+"&lockId="+_obj.lockId+"&startTime="+_obj.startTime+"&endTime="+_obj.endTime,
                //    dataType: "json",
                //    method: "GET",
                //    success: function(resp) {
                //        console.log(resp.data.data.datas.length)
                //        localStorage.historyData=resp.data.data.datas;
                //        console.log("fcg"+localStorage.historyData.length)
                        _bike_obj.init();
                //    }
                //});
        ...
        $(".map-ctrl-goBack").on("click",function(e){
            e.stopPropagation();
            $("#bike_detail").animate({"opacity":0});
            $("#bike_detail").hide();
        })
        $(".detail_panel").on("click",function(e){
            e.stopPropagation();
        })
        $("#toList").on("click",function () {
            if(new Date($("#createTime_to").val().replace(/\-/g, "\/"))<new Date($("#createTime_from").val().replace(/\-/g, "\/"))){$("#s_info").text("起始时间必须小于结束时间");$("#search_rslt").modal('show')}
                ...
                //_obj.startTime=($("#createTime_from").val()).split("/")[2]+"-"+($("#createTime_from").val()).split("/")[0]+"-"+($("#createTime_from").val()).split("/")[1];
                //_obj.endTime=($("#createTime_to").val()).split("/")[2]+"-"+($("#createTime_to").val()).split("/")[0]+"-"+($("#createTime_to").val()).split("/")[1];
                _obj.startTime=$("#createTime_from").val().toString();
                _obj.endTime=$("#createTime_to").val().toString();
                //$.ajax({
                //    url: _defautBiz.api("export")+"?lockId="+_obj.lockId+"&startTime="+_obj.startTime+"&endTime="+_obj.endTime,
                //    dataType: "json",
                //    method: 'get',
                //    success: function(resp) {
                //    }});
                window.open(_defautBiz.api("export")+"?lockId="+_obj.lockId+"&startTime="+_obj.startTime+"&endTime="+_obj.endTime);;
            }
        });
        ...
        for(var i in pt_list){
            ...
            for(var j in pt_list){
                if(pt_list[i]!=pt_list[j]){
                    ...
                    //marker.on("movealong",function(e){alert("fck")})
    ...
    refreshPagination:function(indx){
        _obj.currPage=indx;
        $.ajax({
            url: _defautBiz.api("history_list")+"?pageSize="+_obj.pageSize+"&currPage="+indx+"&lockId="+_obj.lockId+"&startTime="+_obj.startTime+"&endTime="+_obj.endTime,
            dataType: "json",
            method: "GET",
            success: function(resp) {
                $("#Pagination tbody").html("");
    ...
    refreshHistory:function(){
        ...
        _obj.currPage=1;
        $.ajax({
            url: _defautBiz.api("history_list")+"?pageSize="+_obj.pageSize+"&currPage="+1+"&lockId="+_obj.lockId+"&startTime="+_obj.startTime+"&endTime="+_obj.endTime,
            dataType: "json",
            method: "GET",
            success: function(resp) {
                $("#Pagination tbody").html("");
                ...
                $("#Pagination tbody").append(data_append);
                $('.M-box1').pagination({
                    totalData:resp.data.data.total,
                    showData:_obj.pageSize,
                    jump:true,
                    coping:true,
                    callback:function(index){
                        var num=index.getCurrent();
...
var _bike_obj={
    ...
    line_period:0,
    init:function(){
        this.lockId=$("#lockId").val();
        this.startTime=$("#createTime_from").val().toString();
        this.endTime=$("#createTime_to").val().toString();
        //this.startTime=($("#createTime_from").val()).split("/")[2]+"-"+($("#createTime_from").val()).split("/")[0]+"-"+($("#createTime_from").val()).split("/")[1];
        //this.endTime=($("#createTime_to").val()).split("/")[2]+"-"+($("#createTime_to").val()).split("/")[0]+"-"+($("#createTime_to").val()).split("/")[1];
        $("#lock_name").text("");
        $("#lock_state").text("");
        $("#sys_date").text("");
        $("#pos").text("");
    ...
    componentInit:function(){
        ...
        $(".mp-period").off().on("click",function(){
            //if($(this).hasClass("active")){$(".button_panel").hide();$(".mp-period").removeClass("active");}
            //else {$(this).addClass("active");$(".button_panel").show()}
        });
        $("#detail_goBack").on("click",function(){
            $("#bike_detail").animate({"opacity":0});
            $("#bike_detail").hide();
        ...
        $("#map-ctrl-satellite").on("click",function(){
            //window.history.go(-1);
            if(!$(this).hasClass("active")){$("#map-ctrl-satellite span").text("地图");$(this).addClass("active");_obj.satellite=[new AMap.TileLayer.Satellite(),new AMap.TileLayer.RoadNet()];_bk_obj.map.setLayers(_obj.satellite)}
            else {$("#map-ctrl-satellite span").text("卫星图");$(this).removeClass("active");_bk_obj.map.setLayers([new AMap.TileLayer()])}
            //_obj.massBuild=false;
            //_obj.drawMass();
        })
        $("#begin").on("click",function() {
            $(this).addClass("active");
            $("#pause,#end").removeClass("active");
            clearInterval(_bike_obj.runInterval);
            _bike_obj.runInterval=setInterval(function(){
                if(_bike_obj.pointsLine[_bike_obj.intervalIndx]){
                    _bike_obj.lineMarker.setPosition([_bike_obj.pointsLine[_bike_obj.intervalIndx].lng,_bike_obj.pointsLine[_bike_obj.intervalIndx].lat]);
                        $("#lock_name").text(_bike_obj.pointsLine[_bike_obj.intervalIndx].lockName);
                        $("#lock_state").text(_bike_obj.pointsLine[_bike_obj.intervalIndx].status);
                        $("#sys_date").text(_bike_obj.pointsLine[_bike_obj.intervalIndx].sysTimeStr);
                        $("#pos").text(_bike_obj.pointsLine[_bike_obj.intervalIndx].location);
                        $("#alarm").text(_bike_obj.pointsLine[_bike_obj.intervalIndx].warnInfo);
                        $("#power").text(_bike_obj.pointsLine[_bike_obj.intervalIndx].power);
                        $("#open_time").text(_bike_obj.pointsLine[_bike_obj.intervalIndx].openLockCount);
                        $("#geo_type").text(_bike_obj.pointsLine[_bike_obj.intervalIndx].locationModel);
                        _bike_obj.intervalIndx++;
                        if(_bike_obj.intervalIndx==(_bike_obj.pointsLine.length))clearInterval(_bike_obj.runInterval);
                }
                else{
                    _bike_obj.intervalIndx=0;
                }
            },500)
            //    _bike_obj.lineMarker.moveAlong(_bike_obj.moveLine, 100000);
        });
                //var Timeouhistory=setTimeout(function(){_bike_obj.lineMarker.setPosition([_bike_obj.pointsLine[i].lng,_bike_obj.pointsLine[i].lat])},500);
            //})

        //});
        $("#pause").on("click",function(){
            $(this).addClass("active");
            $("#begin,#end").removeClass("active");
            clearInterval(_bike_obj.runInterval);
        });
        $("#end").on("click",function(){
            $(this).addClass("active");
            $("#pause,#begin").removeClass("active");
            _bike_obj.intervalIndx=0;
            clearInterval(_bike_obj.runInterval);
        });
        $("#show_info").on("click",function(){
            if($(this).hasClass("active")){$(".button_panel").hide();$(this).removeClass("active");}
            else {$(this).addClass("active");$(".button_panel").show()}
            //_bike_obj.lineMarker.stopMove();
        });
    },
    drawMap:function(){
    ...
    drawHistory:function(){
        _bike_obj=this;
        _bike_obj.markerList=[];
        var lineArr = [],lineAll=[];
        _bike_obj.lockId=$("#lockId").val();
        _bike_obj.startTime=$("#createTime_from").val().toString();
        _bike_obj.endTime=$("#createTime_to").val().toString();
        //_bike_obj.startTime=($("#createTime_from").val()).split("/")[2]+"-"+($("#createTime_from").val()).split("/")[0]+"-"+($("#createTime_from").val()).split("/")[1];
        //_bike_obj.endTime=($("#createTime_to").val()).split("/")[2]+"-"+($("#createTime_to").val()).split("/")[0]+"-"+($("#createTime_to").val()).split("/")[1];
        $("#bike_detail").show();
        $("#bike_detail").animate({"opacity":1});
        $.ajax({
            //url: _defautBiz.api("history_list")+"?pageSize="+10000+"&currPage="+1+"&lockId="+_bike_obj.lockId+"&startTime="+_bike_obj.startTime+"&endTime="+_bike_obj.endTime,
            url: _defautBiz.api("history_list")+"?pageSize="+10000+"&currPage="+1+"&lockId="+_bike_obj.lockId+"&startTime="+_bike_obj.startTime+"&endTime="+_bike_obj.endTime,
            dataType: "json",
            method: "GET",
            success: function(resp) {
                _bike_obj.pointsLine=$.grep(resp.data.data.datas,function(e,i){return e.lat&&e.lng}).reverse();
                for(var i in resp.data.data.datas){
                    if(resp.data.data.datas[i].lat&&resp.data.data.datas[i].lng){
                        //var marker =_bike_obj.lineMarker= new AMap.Marker({
                        //    map: _bike_obj.map,
                        //    position:[resp.data.data.datas[i].lng,resp.data.data.datas[i].lat],
                        //    icon: "/static/qbike/img/in_trouble.png",
                        //    offset: new AMap.Pixel(-26, -22),
                        //    autoRotation: true
                        //});
                        //marker.num=i;
                        //marker.lockName=resp.data.data.datas[i].lockId;
                        //marker.status=resp.data.data.datas[i].status;
                        //marker.sysTimeStr=resp.data.data.datas[i].sysTimeStr;
                        //marker.location=resp.data.data.datas[i].location;
                        //marker.warnInfo=resp.data.data.datas[i].warnInfo;
                        //marker.power=resp.data.data.datas[i].power;
                        //marker.openLockCount=resp.data.data.datas[i].openLockCount;
                        //marker.locationModel=resp.data.data.datas[i].locationModel;
                        //marker.on("click",function(){
                        //    if(_bike_obj.markerList.length>0)for(var i in _bike_obj.markerList){
                        //        _bike_obj.markerList[i].setIcon("/static/qbike/img/in_trouble.png");
                        //        _bike_obj.markerList[i].setOffset(new AMap.Pixel(-26, -22));
                        //    }
                        //    this.setIcon("/static/qbike/img/in_trouble_Max.png");
                        //    this.setOffset(new AMap.Pixel(-33, -29));
                        //    $("#lock_name").text(this.lockName);
                        //    $("#lock_state").text(this.status);
                        //    $("#sys_date").text(this.sysTimeStr);
                        //    $("#pos").text(this.location);
                        //    $("#alarm").text(this.warnInfo);
                        //    $("#power").text(this.power);
                        //    $("#open_time").text(this.openLockCount);
                        //    $("#geo_type").text(this.locationModel);
                        //
                        //});
                        //_bike_obj.markerList.push(marker);
                        lineArr.push([resp.data.data.datas[i].lng,resp.data.data.datas[i].lat]);
                    }
                }
                console.log(lineArr);
                if(lineArr.length>0)
                    var marker =_bike_obj.lineMarker= new AMap.Marker({
                        map: _bike_obj.map,
                        position:lineArr[0],
                        icon: "/static/qbike/img/total.png",
                        offset: new AMap.Pixel(-26, -22),
                        autoRotation: false
                    });
                _bike_obj.map.setZoomAndCenter(15,lineArr[0])
                console.log(lineArr[0]);
                console.log(marker.getPosition().I+":"+marker.getPosition().M);
                _bike_obj.moveLine=lineArr;
                var polyline = new AMap.Polyline({
    ...
    drawDemo:function(){
        ...
        var marker, lineArr = [],lineAll=[];
        marker =_bike_obj.lineMarker= new AMap.Marker({
            map: _bike_obj.map,
        ...
        polyline.setMap(_bike_obj.map);
        lineArr=[[116.397428, 39.90923]];
        lineAll.push([116.397428, 39.90923]);
        for (var i = 1; i < 4; i++) {
            lngX = lngX + Math.random() * 0.05;
            if (i % 2) {
                latY = latY + Math.random() * 0.0001;
        ...
        // });
        polyline.setMap(_bike_obj.map);
        var polylinenone = new AMap.Polyline({
            path: _bike_obj.moveLine, //设置线覆盖物路径
            strokeColor: "#08e528", //线颜色
            strokeOpacity: 1, //线透明度
            strokeWeight: 5, //线宽
            strokeStyle: "solid", //线样式
    ...
    drawline:function(){
        ...
        $.ajax({
            url: _defautBiz.api("list") + "?bikeId=" + localStorage.bike_id + "&startTime="+startTime+"&endTime="+new Date().format("yyyyMMddhhmmss"),
            ...
            success: function(resp) {
                ...
                $.ajax({
                    ...
                    success: function(resp) {
                        ...
                        _bike_obj.map.setZoomAndCenter(14, [resp.data.lng,resp.data.lat]);
...
//    history_obj.refreshData();
//history_obj.map.setMapStyle('normal');
//},10000);
//function Data_refresh(){
//    history_obj.refreshData();
//    setTimeout(function(){Data_refresh()},10000)
//}
history_obj.Data_refresh();
history_obj.KillInterval();
