...
Vcity.CitySelector.prototype = {
    ...
    createWarp:function(){
        ...
        $("#bike-num").on("click",function(){
            ...
            dispatch_obj.map.setZoomAndCenter(3, [102.342785, 35.312316]);
            //dispatch_obj.dataUpdate($(".select-panel.active a").attr("id"));
        });
        $(".hot-city-item").on("click",function(){
            ...
            dispatch_obj.geocoder.getLocation($(this).text().split("(")[0], function (status, result) {
                ...
                dispatch_obj.map.setZoomAndCenter(level, [geocode[0].location.getLng(), geocode[0].location.getLat()]);
            });
        });
    },
    ...
    linkEvent:function(){
        ...
        for(var i=0,n=links.length;i<n;i++){
            links[i].onclick = function(){
                ...
                dispatch_obj.geocoder.getLocation(this.innerHTML, function (status, result) {
                        var geocode = result.geocodes;
                        //addMarker(0, geocode[0]);
                        var marker = new AMap.Marker({
                            map: null,
    ...
    liEvent:function(){
        ...
        for(var i = 0,n = lis.length;i < n;i++){
            Vcity._m.on(lis[i],'click',function(event){
                ...
                //$("#bike-num").text(that.input.value);
                dispatch_obj.geocoder.getLocation(that.input.value, function (status, result) {
                    var geocode = result.geocodes;
                    //addMarker(0, geocode[0]);
                    var marker = new AMap.Marker({
                        map: null,
