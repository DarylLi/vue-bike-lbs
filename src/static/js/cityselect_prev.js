/**
 * Created by lihaotian on 2016/12/19.
 */
/* *
 * 全局空间 Vcity
 * */
...
$.ajax({
    url: "/bike/putArea/list.json",
    dataType: "json",
    method: 'get',
    success: function(resp) {
        for(var i in resp.data){
            Vcity.allCity.push((resp.data[i].areaname+'|'+resp.data[i].areacode+'|null|'+resp.data[i].right))
        }
...
/* 城市HTML模板 */
Vcity._template = [
    '<p class="tip">当前城市: <span id="c_city">全国</span></p>',
    '<ul>',
    '<li class="on" id="bike-num" areacode="0">全国</li>',
    '</ul>',
    '<ul class="city-header">',
    '<li class="on" style="width: calc(100% - 10px)">热点城市<span class="hot-tip" style="float: right; color: red; display: none">您无权限查阅该区域的信息</span></li>',
    '</ul>',
    '<div id="main-city">',
    '</div>',
    '<ul class="city-header">',
    '<li class="on" style="width: calc(100% - 10px)">已投放城市<span class="city-tip" style="float: right; color: red;display: none">您无权限查阅该区域的信息</span></li>',
    '</ul>',
    //'<ul class="text-indx">',
    //'<li>AB</li>',
    //'<li>CD</li>',
...
Vcity.CitySelector.prototype = {
    ...
    createWarp:function(){
        ...
        this.createHotCity();
        //$("#main-city").append('<div class="hot cityTab"><dl><dd><a href="#" class="hot-city-item">上海(30,000辆)</a><a href="#" class="hot-city-item">杭州(30,000辆)</a><a href="#" class="hot-city-item">南京(30,000辆)</a><a href="#" class="hot-city-item">苏州(30,000辆)</a><a href="#" class="hot-city-item">成都(30,000辆)</a><a href="#" class="hot-city-item">北京(30,000辆)</a></dd></dl></div>');
        var city_list='';
        $("#main-city").append('<div class="hot cityTab"><dl><dd></dd></dl></div>');
        $.ajax({
            url: "/bike/putArea/list.json",
            dataType: "json",
            method: 'get',
            success: function(resp) {
                for (var i in resp.data) {
                    //Vcity.allCity.push((resp.data[i].areaname + '|null|null'))
                    //city_list+=;
                    // $("#main-city dd").append('<a href="javascript:void(0)" class="hot-city-item" areacode="'+resp.data[i].areacode+'">'+resp.data[i].areaname.split("市")[0]+'</a>');
                    $("#main-city dd").append('<a href="javascript:void(0)" class="hot-city-item" areacode="'+resp.data[i].areacode+'" data-status="'+resp.data[i].right+'">'+resp.data[i].areaname+'</a>');
                }
        //$("#main-city").append('<div class="hot cityTab"><dl><dd><a href="javascript:void(0)" class="hot-city-item">上海</a></dd></dl></div>');
        $("#c_city").html(localStorage.c_city_name);
        $("#bike-num").on("click",function(){
            $("#c_city").text($("#bike-num").text().split("(")[0]);
            $("#citySelect").val($("#bike-num").text().split("(")[0]);
            $("#citySelect").attr("areacode",$("#bike-num").attr("areacode"));
            superise_obj.map.setZoomAndCenter(3, [102.342785, 35.312316]);
            superise_obj.lastTimestamp_useIndx_h="";
            superise_obj.lastTimestamp_useIndx_m="";
            superise_obj.lastTimestamp_income_h="";
            superise_obj.lastTimestamp_income_m="";
            superise_obj.refreshData();
            superise_obj.mapPointsUpdate();
            $(".cityBox").addClass("hide");
        });
        $(".hot-city-item").on("mouseover", function(){
            if($(this).attr("data-status") == "false"){
                $('.hot-tip').show();
            }
        })

        $(".hot-city-item").on("mouseout", function() {
            $('.hot-tip').hide();
        })

        $(".hot-city-item").on("click",function(){
            if($(this).attr("data-status") == "false") return

            $("#citySelect").val($(this).text().split("(")[0].substring(0,2));
            $("#citySelect").attr("areacode",$(this).attr("areacode"));
            localStorage.c_city_name=$(this).text().split("(")[0]
            localStorage.c_areacode=$("#citySelect").attr("areacode");
            $("#c_city").text(localStorage.c_city_name);
            //$("#bike-num").text($(this).text());
            superise_obj.geocoder.getLocation($(this).text().split("(")[0], function (status, result) {
                ...
                superise_obj.refreshData();
                //superise_obj.mapPointsUpdate();
                superise_obj.searching_lng=geocode[0].location.getLng();
                superise_obj.searching_lat=geocode[0].location.getLat();
                superise_obj.map.setZoomAndCenter(level, [geocode[0].location.getLng(), geocode[0].location.getLat()]);
            });
        });
            }
        });
        ...
        for(key in oCity){
            ...
            for(var j=0,k = sortKey.length;j<k;j++){
                ...
                for(var i=0,n=oCity[key][sortKey[j]].length;i<n;i++){
                    var p = Vcity.allCity[i].split("|")
                    str = '<a href="javascript:void(0)" areacode="'+p[1]+'" data-status="'+p[3]+'">' + oCity[key][sortKey[j]][i] + '</a>';
                    odda.push(str);
                }
    ...
    linkEvent:function(){
        ...
        for(var i=0,n=links.length;i<n;i++){
            ...
            links[i].onclick = function(){
                if($(this).attr("data-status") == "false") return
                //debugger;
                //console.log($(this).attr("areacode"));
                    that.input.value = this.innerHTML.split("市")[0].length>2?(this.innerHTML.split("市")[0].substring(0,2)+"..."):this.innerHTML.split("市")[0];
                    //console.log($(that.input).attr(""));
                    $(that.input).attr("areacode",$(this).attr("areacode"))
                    superise_obj.geocoder.getLocation(this.innerHTML, function (status, result) {
                        var geocode = result.geocodes;
                        //addMarker(0, geocode[0]);
                        var marker = new AMap.Marker({
                            map: null,
                ...
                localStorage.c_city_name=this.innerHTML.split("市")[0]
                localStorage.c_areacode=$(this).attr("areacode");
                $("#c_city").html(localStorage.c_city_name);
                //$("#bike-num").text(that.input.value);
            ...
            for (var i = 0, n = Vcity.allCity.length; i < n; i++) {
                if (reg.test(Vcity.allCity[i])) {
                    ...
                    if (searchResult.length !== 0) {
                        str = '<li><b class="cityname">' + match[1] + '</b>';
                    } else {
                        str = '<li class="on"><b class="cityname" areacode="'+match[2]+'">' + match[1] + '</b><b class="cityspell">' + match[2] + '</b></li>';
                    }
    ...
    liEvent:function(){
        ...
        for(var i = 0,n = lis.length;i < n;i++){
            Vcity._m.on(lis[i],'click',function(event){
                ...
                event = Vcity._m.getEvent(event);
                var target = Vcity._m.getTarget(event);
                localStorage.c_city_name=Vcity.regExChiese.exec(target.innerHTML)[0];
                $("#c_city").html(localStorage.c_city_name);
                that.input.value = (Vcity.regExChiese.exec(target.innerHTML)[0]).split("市")[0].length>2?((Vcity.regExChiese.exec(target.innerHTML)[0]).split("市")[0].substring(0,2)+"..."):(Vcity.regExChiese.exec(target.innerHTML)[0]).split("市")[0];
                //$("#bike-num").text(that.input.value);
                // console.log($(target).attr("areacode"));
                //$(that.input).attr("areacode",$(target).attr("areacode"))
                superise_obj.geocoder.getLocation(that.input.value, function (status, result) {
                    var geocode = result.geocodes;
                    //addMarker(0, geocode[0]);
                    var marker = new AMap.Marker({
                        map: null,
