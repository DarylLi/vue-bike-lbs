...
$.ajax({
    ...
    success: function(resp) {
        for(var i in resp.data){
            alert_obj.areas=alert_obj.areas+(resp.data[i].right?(resp.data[i].areacode+","):"");
            Vcity.allCity.push((resp.data[i].areaname+'|'+resp.data[i].areacode+'|'+resp.data[i].right))
        }
...
Vcity._template = [
    ...
    '<li class="on" style="width: calc(100% - 10px)">预警城市<span class="hot-tip" style="float: right; color: red; display: none">您无权限查阅该区域的信息</span></li>',
    '</ul>',
    //'<ul class="text-indx">',
    //'<li>AB</li>',
    //'<li>CD</li>',
...
Vcity.CitySelector.prototype = {
    ...
    createWarp:function(){
        ...
        $("#c_city").html(localStorage.alert_city);
        $("#bike-num").on("click",function(){
            ...
            localStorage.ac_areacode=$("#citySelect").attr("areacode");
            localStorage.ac_city_name=$("#bike-num").text().split("(")[0];
            $("#c_city").text(localStorage.ac_city_name);
            $(".cityBox").addClass("hide");
            alert_obj.map.setZoomAndCenter(3, [102.342785, 35.312316]);
            alert_obj.dataUpdate($(".select-panel.active a").attr("id"));
        });
        $(".hot-city-item").on("click",function(){

            $("#citySelect").val($(this).text().split("(")[0].substring(0,2)+"...");
            localStorage.ac_city_name=$(this).text().split("(")[0]
            $("#c_city").text(localStorage.ac_city_name);
        ...
        for(key in oCity){
            ...
            for(var j=0,k = sortKey.length;j<k;j++){
                ...
                for(var i=0,n=oCity[key][sortKey[j]].length;i<n;i++){
                    str = '<a href="javascript:void(0)" areacode="'+Vcity.allCity[i].split("|")[1]+'" data-status="'+Vcity.allCity[i].split("|")[2]+'">' + oCity[key][sortKey[j]][i] + '</a>';
                    odda.push(str);
                }
    ...
    linkEvent:function(){
        ...
        var that = this;
        $(".cityTab a").on("mouseover", function(){
            if($(this).attr("data-status") == "false"){
                $('.hot-tip').show();
            }
        })

        $(".cityTab a").on("mouseout", function() {
            $('.hot-tip').hide();
        })
        for(var i=0,n=links.length;i<n;i++){
            links[i].onclick = function(){
                ...
                alert_obj.geocoder.getLocation(this.innerHTML, function (status, result) {
                        var geocode = result.geocodes;
                        //addMarker(0, geocode[0]);
                        var marker = new AMap.Marker({
                            map: null,
                    ...
                    alert_obj.searching_lat=geocode[0].location.getLat();
                    alert_obj.searching_lng=geocode[0].location.getLng();
                    alert_obj.map.setZoomAndCenter(level, [geocode[0].location.getLng(), geocode[0].location.getLat()]);
                    alert_obj.dataUpdate($(".select-panel.active a").attr("id"));
                    });
                localStorage.ac_city_name=this.innerHTML.split("市")[0];
                localStorage.ac_areacode=$(this).attr("areacode");
                $("#c_city").html(localStorage.ac_city_name);
    ...
    liEvent:function(){
        ...
        for(var i = 0,n = lis.length;i < n;i++){
            Vcity._m.on(lis[i],'click',function(event){
                event = Vcity._m.getEvent(event);
                var target = Vcity._m.getTarget(event);
                localStorage.ac_city_name=Vcity.regExChiese.exec(target.innerHTML)[0]
                $("#c_city").html(localStorage.ac_city_name);
                ...
                //$("#bike-num").text(that.input.value);
                alert_obj.geocoder.getLocation(that.input.value, function (status, result) {
                    var geocode = result.geocodes;
                    //addMarker(0, geocode[0]);
                    var marker = new AMap.Marker({
                        map: null,
